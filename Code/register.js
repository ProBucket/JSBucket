jQuery(document).ready(function(){
    ...
    $('input:radio[name="data[Delegate][title]"]').change(function(){
        if ($(this).is(':checked') && $(this).val() == 'Other') {
            $('.title_other').removeClass('hidden');
        } else {
            if(!$('.title_other').hasClass('hidden')){
    ...
    $(".admin_complementary").change(function() {
        if(this.checked) {
            $('#card_section').hide();
            $('#admin-complementary-selected').val(1);
        } else {
            $('#card_section').show();
            $('#admin-complementary-selected').val(0);
        }
    });

    $(".common_check").change(function() {
        if(this.checked) {
           $(this).closest('.register_amount').find('input').prop('required',true);
        } else {
            $(this).closest('.register_amount').find('input').removeAttr('required');
        }
    });
        ...
        $.ajax({
            ...
            success:function(response){
                ...
                if(obj.status == 'Error'){
                    ...
                    $('#orderSuccessPopupMessage').hide();
                    $('.btnsubmit').attr('disabled',false);
                    $('#error_msg').closest('.alert').removeClass('hidden');
                    $('#error_msg').html(obj.msg);

                    $('html, body').animate({
                        scrollTop: $(".event-location-top").offset().top
                    }, 3000);

                } else if(obj.status == 'Success'){
                    window.location= SITE_URL + 'workshops/success';
                }
            }
        });
    }

    $("body").delegate('.registration_qty', "change keyup", function() {
        //console.log('change');
        var qty = $(this).val();
        //if(qty>0) {
            ...
            // 15/11/2016
            if ((mm == 11 && dd > 15) || mm > 11) {
                var amount = $this_closest.find('.register_standard').find('label').text();

                if (qty > 0) {
                    //console.log(1);
                    $($this_closest).find('.register_standard').find('input').prop('checked', true);
                } else {
                    //console.log(2);
                    $($this_closest).find('.register_standard').find('input').prop('checked', false);
                    $($this_closest).find('.registration_fee_item_total').html('$');
                }
                ...
                if (qty > 0) {
                    //console.log(3);
                    $($this_closest).find('.register_early_bird').find('input').prop('checked', true);
                } else {
                    //console.log(4);
                    $($this_closest).find('.register_early_bird').find('input').prop('checked', false);
                    $($this_closest).find('.registration_fee_item_total').html('$');
                }
